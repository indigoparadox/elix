...
#define CHECK_MID_DELETEME 99
#define CHECK_SZ_DELETEME 20
char g_chk_deleteme[CHECK_SZ_DELETEME] = "Delete this string.";
...
static void setup_mem() {
   ...
   mem_ptr = mget( CHECK_PID, CHECK_MID_DELETEME, CHECK_SZ_DELETEME );
   mcopy( mem_ptr, g_chk_deleteme, CHECK_SZ_DELETEME );
   mfree( CHECK_PID, CHECK_MID_DELETEME );
...
START_TEST( test_vars ) {
   ...
   //ck_assert_int_eq( var_header->sz, g_chk_len[_i] );
...
START_TEST( test_mshift ) {
   ...
   ck_assert_int_eq( var_header->sz, g_chk_len[0] );
...
START_TEST( test_mget_pid_match ) {
   ...
   ck_assert_int_eq( var_header->mid, CHECK_MID_TEST1 );
   ck_assert_int_eq( var_header->sz, CHECK_STR_SZ );
   ...
   ck_assert_int_eq( var_header->pid, CHECK_PID );
   ck_assert_int_eq( var_header->mid, CHECK_MID_TEST1 );
   ck_assert_int_eq( var_header->sz, CHECK_STR_SZ_2 );
   offset += sizeof( struct mvar );
