...
static const char gc_stream_error[] = "EVMS01\n";
#else
...
static SIPC_PTR vm_sysc_mfat( TASK_PID pid, uint8_t call_id ) {
   ...
   part_id = vm_stack_pop( &(task->state) );
   if( 0 > part_id ) { return -1; }
   disk_id = vm_stack_pop( &(task->state) );
   ...
   case VM_SYSC_DFIRST:
      offset = vm_stack_dpop( &(task->state) );
   ...
   if( 0 > vm_stack_dpush( &(task->state), offset ) ) {
      return -1;
   }
...
SIPC_PTR vm_instr_sysc(
   TASK_PID pid, struct mvm_state* state, uint8_t call_id
) {
   struct adhd_task* task = &(g_tasks[pid]);
   unsigned char cbuf = 0;
   char* str_ptr = NULL;
   ...
   case VM_SYSC_GETC:
      if( task->flags & ADHD_TASK_FLAG_FOREGROUND ) {
         cbuf = tgetc();
         if( 0 > vm_stack_push( &(task->state), cbuf ) ) {
            return -1;
         }
      } else {
         if( 0 > vm_stack_push( &(task->state), 0 ) ) {
            return -1;
         }
      }
      break;
